...
function CodeEditor(): React.ReactNode {
  const [language, setLanguage] = useState<keyof typeof EXAMPLES>("python");
  const [tabs, setTabs] = useState<Tab[]>([
    { id: 1, name: `main${LANGUAGES.python.extension}`, content: LANGUAGES.python.template },
  ]);
  const [activeTab, setActiveTab] = useState(1);
  const [consoleMode, setConsoleMode] = useState("input");
  ...
  function handleLanguageChange(newLang: keyof typeof EXAMPLES) {
    setLanguage(newLang);
    setSelectedExample("");
    ...
    setTabs([
      {
        id: 1,
        name: `main${LANGUAGES[newLang].extension}`,
        content: LANGUAGES[newLang].template,
      },
    ]);
    setActiveTab(1);
  }
  ...
  const addNewTab = () => {
    ...
    const newTab = {
      id: newId,
      name: `file${newId}${LANGUAGES[language].extension}`,
      content: "",
    };
    setTabs([...tabs, newTab]);
    setActiveTab(newId);
    ...
    <div className="flex flex-col h-screen bg-gray-900 text-gray-100">
      ...
      <div className="flex items-center gap-2 p-2 bg-gray-800 border-b border-gray-700">
        ...
        <button
          ...
          disabled={isRunning}
          className="flex items-center gap-2 px-4 py-1.5 bg-green-600 hover:bg-green-700 disabled:bg-gray-600 disabled:cursor-not-allowed rounded text-sm font-medium transition-colors"
        >
          {isRunning ? (
            <>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {consoleMode === "input" ? (
              <div className="flex-1 flex flex-col">
                <div className="p-2 bg-gray-800 border-b border-gray-700 text-sm font-medium">Standard Input</div>
                <textarea
                  value={stdinInput}
              ...
              <div className="flex-1 flex flex-col">
                <div className="p-2 bg-gray-800 border-b border-gray-700 text-sm font-medium">Output Console</div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-80">
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={handleTabNameChange} className="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded">
                Save
              </button>
              <button
